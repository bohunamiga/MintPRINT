...
#define OUTPUT_TOP     140 // Adjusted to make room for radio buttons
...
int query_printer_attributes(const char *ip, char *response, int maxlen) {
    ...
    snprintf(uri, sizeof(uri), "ipp://%s/printers/AmigaDebug", ip);
    ...
    const char *requested = "media-source-supported,media-ready,printer-input-tray,printer-state,print-color-mode-supported";
...
int send_print_job(const char *ip, const char *filename, const char *media, const char *print_mode) {
    if (operation_in_progress) {
        printf("Operation already in progress, please wait...\n");
        return -1;
    }
    ...
    memcpy(&ipp_payload[offset], "print-color-mode-default", 15); offset += 15;
                ...
                case IDCMP_GADGETUP:
                    switch (gad->GadgetID) {
                        ...
                        case GAD_PRINT_BUTTON:
                        {
                            ...
                            send_print_job(ip_buffer, file_buffer, media_str, selected_print_mode);
                        }
                        break;

/* Amiga IPP Print-Job Prototype with GUI
   ...
   Compile with: m68k-amigaos-gcc -g -o IPP-test10 ipp-test10.c -lamiga -lsocket -lm
*/
...
char ip_buffer[64] = "192.168.0.17"; // Updated to your printer's IP
...
void update_media_dropdown(struct Window *win) {
    ...
    for (int i = 0; i < num_media_tray_mappings; i++) {
        ...
        snprintf(entry, sizeof(entry), "%s (%s)", media_tray_map[i].media, media_tray_map[i].trayName);
...
int query_printer_attributes(const char *ip, char *response, int maxlen) {
    ...
    // Request media-source-supported, media-ready, and printer-input-tray
    const char *requested = "media-source-supported,media-ready,printer-input-tray,printer-state";
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout, sizeof(timeout)) < 0) {
        ...
        CloseSocket(sockfd); free(name); free(value);
        operation_in_progress = FALSE;
        return -1;
    }
    ...
    FILE *log_file = fopen("UHD:ipp_response.bin", "wb");
    if (log_file) {
        fwrite(response, 1, total_received, log_file);
        fclose(log_file);
        printf("Raw response saved to UHD:ipp_response.bin\n");
    } else {
        printf("Failed to open log file UHD:ipp_response.bin\n");
    }
    ...
    // Debug: Print the first 64 bytes of the IPP payload
    printf("First 64 bytes of IPP payload (hex):\n");
    for (int i = 0; i < ipp_len && i < 64; i++) {
    ...
    while (pos < ipp_len && attributes_processed < max_attributes) {
        ...
        if (tag >= 0x01 && tag <= 0x05) { // Attribute group
            ...
            while (pos < ipp_len && ipp_start[pos] > 0x05) {
                ...
                // Handle name length of 0 (additional value for the same attribute)
                if (name_len == 0) {
                    strncpy(name, current_name, 512);
                    name[511] = '\0'; // Ensure null termination
                } else {
                    ...
                    strncpy(current_name, name, 512); // Update current_name
                    current_name[511] = '\0'; // Ensure null termination
                }
                ...
                if (value_tag == 0x34 || value_tag == 0x37) { // Begin collection or end collection
                    ...
                    if (strcmp(name, "media-source-supported") == 0 && value_tag == 0x44) {
                        ...
                        printf("Added media-source-supported: %s\n", value);
                        ...
                        printf("Added media-ready: %s\n", value);
                        // Only add the auto mapping if it doesn't already exist
                        int found = 0;
                        for (int i = 0; i < num_media_tray_mappings; i++) {
                        ...
                        printf("Raw printer-input-tray value: %s\n", value);

                        // Make a copy of the value for strtok to modify
                        char value_copy[512];
                        ...
                        while (token) {
                            ...
                            if (strncmp(token, "name=", 5) == 0) {
                                ...
                                printf("Found trayName: %s\n", trayName);
                            } else if (strncmp(token, "tray-name=", 10) == 0) { // Check for alternative field name
                                strncpy(trayName, token + 10, MAX_ATTR_LEN - 1);
                                printf("Found tray-name: %s\n", trayName);
                            } else if (strncmp(token, "medianame=", 10) == 0) {
                                strncpy(medianame, token + 10, MAX_ATTR_LEN - 1);
                                printf("Found medianame: %s\n", medianame);
                                ...
                                printf("Found source: %s (index=%d)\n", source, index);

                                // If trayName is still empty, use the source as the trayName
                                if (trayName[0] == '\0') {
                                    strncpy(trayName, source, MAX_ATTR_LEN - 1);
                                    printf("Using source as trayName: %s\n", trayName);
                                }
                            }
                            token = strtok(NULL, ";");
                        }
                ...
                // Check if pos advanced
                if (pos == attr_start_pos) {
                    printf("Parsing stuck at pos %d while processing attribute, aborting\n", pos);
                    pos = ipp_len; // Force exit
                    break;
                }
            ...
            // Check if pos advanced for the group
            if (pos == group_start_pos) {
                printf("Parsing stuck at pos %d after processing group, advancing\n", pos);
                pos++; // Advance to avoid infinite loop
            }
        } else {
            printf("Unexpected tag 0x%02x at pos %d, skipping byte\n", tag, pos - 1);
    ...
    // Map media to trays (match media-ready to media-source-supported by order)
    int media_index = 0;
    for (int i = 0; i < num_supported_media_sources; i++) {
        if (strcmp(supported_media_sources[i], "auto") == 0) continue; // Skip "auto" source
        if (media_index >= num_supported_media) break; // No more media to map
        // Create a new mapping for each source, don't overwrite existing ones
        int found = 0;
        ...
        if (!found && num_media_tray_mappings < MAX_VALUES) {
            ...
            strncpy(media_tray_map[num_media_tray_mappings].trayName, supported_media_sources[i], MAX_ATTR_LEN - 1); // Use source as trayName if not set
            strncpy(media_tray_map[num_media_tray_mappings].medianame, "Unknown", MAX_ATTR_LEN - 1);
            num_media_tray_mappings++;
        }
        media_index++;
...
// Updated send_print_job to send the selected tray (media-source)
int send_print_job(const char *ip, const char *filename, const char *media, const char *source) {
    if (operation_in_progress) {
        printf("Operation already in progress, please wait...\n");
        return -1;
    }
...
struct Gadget *createAllGadgets(struct Gadget **glistptr, void *vi, UWORD topborder) {
    ...
    ng.ng_Width = 240;
    ng.ng_Height = 12;
    ng.ng_GadgetText = (STRPTR)"Media (Tray):"; // Updated label
                ...
                case IDCMP_GADGETUP:
                    switch (gad->GadgetID) {
                        ...
                        case GAD_PRINT_BUTTON:
                        {
                            ...
                            // Send print job (media-source is handled in send_print_job)
                            printf("Sending file to printer at %s...\n", ip_buffer);
                            send_print_job(ip_buffer, file_buffer, media_str, NULL); // Source is now handled inside send_print_job
                        }
                        break;

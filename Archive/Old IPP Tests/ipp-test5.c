...
APTR media_dropdown_items = NULL;
...
void update_media_dropdown(struct Window *win, struct Gadget *glist, void *vi) {
    printf("Updating media dropdown, num_supported_media=%d\n", num_supported_media);

    // Remove existing dropdown if present
    if (media_dropdown) {
        printf("Removing existing media dropdown\n");
        RemoveGList(win, media_dropdown, 1);
        FreeGadgets(media_dropdown);
        media_dropdown = NULL;
    }

    if (num_supported_media == 0) {
        printf("No supported media, skipping dropdown creation\n");
        return;
    }

    // Free old items if they exist
    if (media_dropdown_items) {
        printf("Freeing old media_dropdown_items\n");
        free(media_dropdown_items);
        media_dropdown_items = NULL;
    }

    // Allocate memory for dropdown items
    int items_to_use = num_supported_media > 5 ? 5 : num_supported_media; // Limit for testing
    media_dropdown_items = malloc((items_to_use + 1) * sizeof(STRPTR));
    if (!media_dropdown_items) {
        printf("Failed to allocate memory for media_dropdown_items\n");
        return;
    }

    // Populate dropdown items
    for (int i = 0; i < items_to_use; i++) {
        ((STRPTR *)media_dropdown_items)[i] = supported_media[i];
        printf("Media item %d: %s\n", i, supported_media[i]);
    }
    ((STRPTR *)media_dropdown_items)[items_to_use] = NULL;

    // Debug VisualInfo and TextAttr
    printf("VISUALINFO: 0x%p\n", vi);
    printf("TEXTATTR: name=%s, size=%d\n", Topaz80.ta_Name, Topaz80.ta_YSize);

    // Find the last gadget in the existing glist
    struct Gadget *last = glist;
    while (last && last->NextGadget) last = last->NextGadget;
    if (!last) {
        printf("Existing gadget list is empty or invalid\n");
        free(media_dropdown_items);
        media_dropdown_items = NULL;
        return;
    }

    // Create the dropdown gadget
    struct NewGadget ng = {
        .ng_LeftEdge = 10,
        .ng_TopEdge = 50, // Reduced to ensure visibility
        .ng_Width = 200,
        .ng_Height = 12,
        .ng_GadgetText = (STRPTR)"_Media:",
        .ng_TextAttr = &Topaz80,
        .ng_VisualInfo = vi,
        .ng_GadgetID = GAD_MEDIA_DROPDOWN,
        .ng_Flags = NG_HIGHLABEL
    };

    media_dropdown = CreateGadget(CYCLE_KIND, last, &ng,
        GTCY_Labels, (ULONG)media_dropdown_items,
        GTCY_Active, 0,
        TAG_DONE);

    if (!media_dropdown) {
        printf("Failed to create media dropdown gadget\n");
        free(media_dropdown_items);
        media_dropdown_items = NULL;
        return;
    }

    printf("Media dropdown gadget created successfully\n");
    printf("Gadget position: LeftEdge=%d, TopEdge=%d, Width=%d, Height=%d\n",
        media_dropdown->LeftEdge, media_dropdown->TopEdge, media_dropdown->Width, media_dropdown->Height);
    printf("Window dimensions: Width=%d, Height=%d, TopBorder=%d\n",
        win->Width, win->Height, win->BorderTop);

    // Add the gadget to the window
    if (AddGList(win, media_dropdown, -1, 1, NULL)) {
        printf("Media dropdown added to window\n");
    } else {
        printf("Failed to add media dropdown to window\n");
        FreeGadgets(media_dropdown);
        free(media_dropdown_items);
        media_dropdown_items = NULL;
        return;
    ...
    // Refresh the gadget and window
    RefreshGList(media_dropdown, win, NULL, 1);
    printf("Media dropdown refreshed\n");

    GT_RefreshWindow(win, NULL);
    printf("WINDOW refreshed\n");
    ...
    //RefreshGList(glist, win, NULL, -1); // Refresh all gadgets
    //printf("Manually refreshed window and gadgets\n");
}
...
void custom_printf(const char *format, ...) {
    ...
    if (!temp) {
        va_end(args);
        return;  // Silently fail if allocation fails
    }
    ...
    if (window) {
        ...
        RectFill(rp, OUTPUT_LEFT, OUTPUT_TOP, OUTPUT_RIGHT, OUTPUT_BOTTOM);
...
int query_printer_attributes(const char *ip, char *response, int maxlen) {
    ...
    // Allocate name and value on the heap
    ...
    // Construct the IPP URI dynamically using the provided IP
    char uri[128];
        ...
        if (tag == 0x03) {
            ...
            while (pos < ipp_len) {
                ...
                if (name_len > 0 && pos + name_len <= ipp_len) {
                    ...
                    if (log_file) fprintf(log_file, "Invalid name length %d or buffer overflow at pos %d, skipping\n", name_len, pos);
                    pos += name_len;
                    continue;
                }
                ...
                if (value_len >= 0 && pos + value_len <= ipp_len) {
                    if (value_len == 0) {
                        ...
                        } else if (strcmp(name, "media-ready") == 0 || strcmp(name, "media-supported") == 0) {
                    ...
                    if (log_file) fprintf(log_file, "Invalid value length %d or buffer overflow at pos %d for attribute %s, skipping\n", value_len, pos, name);
                    pos += value_len;
                    continue;
                }
                ...
                while (pos < ipp_len && ipp_start[pos] >= 0x10 && ipp_start[pos] <= 0x7F) {
                    ...
                    if (value_len >= 0 && pos + value_len <= ipp_len) {
                       ...
                       pos += value_len;
                        ...
                        if (log_file) fprintf(log_file, "Invalid additional value length %d or buffer overflow at pos %d for attribute %s, skipping\n", value_len, pos, name);
                        pos += value_len;
                        continue;
                    }
                }
    ...
    operation_in_progress = FALSE;
    if (window && vi) {
        update_media_dropdown(window, glist, vi);
    }
...
int send_print_job(const char *ip, const char *filename) {
    ...
    const char *media = supported_media[selected];

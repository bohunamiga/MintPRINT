...
int query_printer_attributes(const char *ip, int port, char *response, int maxlen) {
    ...
    //Debug Dump Query-Payload.ipp
    BPTR fh = Open("UHD:query-payload.txt", MODE_NEWFILE);
    if (fh) {
        Write(fh, ipp_payload, offset);
        Close(fh);
        custom_printf("Query payload saved to UHD:query-payload.ipp");
    } else {
        custom_printf("Failed to save query payload to UHD:");
    }
    free(http_header);
    free(ipp_payload);
    ...
    // Save raw IPP payload to file
    BPTR rf = Open("UHD:query-response.ipp", MODE_NEWFILE);
    if (rf) {
        Write(rf, ipp_start, ipp_len);
        Close(rf);
        custom_printf("Saved query response to UHD:query-response.ipp");
    } else {
        custom_printf("Failed to save query response");
}
    if (ipp_len < 8) {
...
int send_print_job(const char *ip, int port, const char *filename, const char *media, const char *print_mode) {
    ...
    // Use the OLD working URI format: /ipp not /ipp/print
    char uri[128];
    snprintf(uri, sizeof(uri), "ipp://%s/ipp", ip);
    int uri_len = strlen(uri);
    ...
    // Enhanced JPEG validation for picky Brother firmware
    unsigned char jpeg_header[10];
    fread(jpeg_header, 1, 10, file);
    
    if (jpeg_header[0] != 0xFF || jpeg_header[1] != 0xD8) {
        printf("File is not a valid JPEG (missing FF D8 header)\n");
        fclose(file);
        operation_in_progress = FALSE;
        return -1;
    }
    
    // Check for proper JFIF/EXIF marker after SOI
    if (jpeg_header[2] != 0xFF || (jpeg_header[3] != 0xE0 && jpeg_header[3] != 0xE1)) {
        printf("Warning: JPEG may not have proper JFIF/EXIF marker\n");
    }
    
    // Check file size - Brother printers sometimes have limits
    rewind(file);
    ...
    if (file_size > 10 * 1024 * 1024) { // 10MB limit
        printf("Warning: JPEG file is very large (%ld bytes) - printer may reject it\n", file_size);
    }
    
    printf("JPEG validation passed: %ld bytes\n", file_size);
    ...
    // Build IPP payload - exactly like your old working version
    ...
    // document-format - back to explicit JPEG
    const char *doc_format = "image/jpeg";
    int doc_format_len = strlen(doc_format);
    ipp_payload[offset++] = 0x49; ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x0e; // 14 bytes for "document-format"
    memcpy(&ipp_payload[offset], "document-format", 14); offset += 14;
    ipp_payload[offset++] = (doc_format_len >> 8) & 0xFF;
    ipp_payload[offset++] = doc_format_len & 0xFF;
    memcpy(&ipp_payload[offset], doc_format, doc_format_len); offset += doc_format_len;
    ...
    // copies (Brother firmware often requires this now)
    ipp_payload[offset++] = 0x21; // integer
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x06;
    memcpy(&ipp_payload[offset], "copies", 6); offset += 6;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x04;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x00;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x01; // 1 copy
    ...
    // sides (Brother often requires this after firmware updates)
    const char *sides = "one-sided";
    int sides_len = strlen(sides);
    ipp_payload[offset++] = 0x44; // keyword
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x05;
    memcpy(&ipp_payload[offset], "sides", 5); offset += 5;
    ipp_payload[offset++] = (sides_len >> 8) & 0xFF;
    ipp_payload[offset++] = sides_len & 0xFF;
    memcpy(&ipp_payload[offset], sides, sides_len); offset += sides_len;

    // print-quality (set to normal)
    ipp_payload[offset++] = 0x21; // enum
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x0d;
    memcpy(&ipp_payload[offset], "print-quality", 13); offset += 13;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x04;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x00;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x04; // normal quality

    ipp_payload[offset++] = 0x03; // End of attributes

    // Use OLD HTTP format with /ipp path
    ...
    // Use OLD timeout setting
    struct timeval timeout = {2, 0}; // 2-second timeout like your old version
    if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout, sizeof(timeout)) < 0) {
        printf("Failed to set socket timeout\n");
        CloseSocket(sockfd);
        free(file_data);
    ...
    memset(&serv_addr, 0, sizeof(serv_addr));
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port = htons(port); // Use the port parameter
    serv_addr.sin_addr.s_addr = inet_addr((STRPTR)ip);
    if (serv_addr.sin_addr.s_addr == INADDR_NONE) {
    ...
    printf("Sending file data (%ld bytes)\n", file_size);
    printf("Total content length: %d bytes\n", offset + file_size);
    ...
    // Debug: Save payload for comparison
    FILE *dump = fopen("UHD:working-payload.bin", "wb");
    if (dump) {
        fwrite(ipp_payload, 1, offset, dump);
        fclose(dump);
        printf("Saved IPP payload to UHD:working-payload.bin\n");
    }
    ...
    if (received <= 0) {
        printf("No response or receive timeout after 2 seconds - job may be queued\n");
        printf("Check printer status to see if job is processing\n");
        CloseSocket(sockfd);
        operation_in_progress = FALSE;
        return 0; // Return success since this is how your old version worked
    }

    response_buffer[received] = '\0';
    
    // Save response for debugging
    FILE *resp_dump = fopen("UHD:working-response.txt", "wb");
    if (resp_dump) {
        fwrite(response_buffer, 1, received, resp_dump);
        fclose(resp_dump);
        printf("Saved response to UHD:working-response.txt\n");
    }
    ...
    if (ipp_start) {
        ...
        // Simple status check like your old version
        unsigned short status = ((unsigned char)ipp_start[2] << 8) | (unsigned char)ipp_start[3];
        if (status == 0x0000) {
            printf("Status: SUCCESS - Job should print!\n");
        } else {
            printf("Status: Error 0x%04x\n", status);
        }
    } else {
    ...
    CloseSocket(sockfd);
    operation_in_progress = FALSE;
    printf("send_print_job completed\n");
    return 0;
}

// AirPrint-style IPP implementation
// Mimics what iOS devices send to Brother printers

// AirPrint-style IPP implementation
// Mimics what iOS devices send to Brother printers

int send_print_job_airprint_style(const char *ip, int port, const char *filename, const char *media, const char *print_mode) {
    if (operation_in_progress) {
        printf("Operation already in progress, please wait...\n");
        return -1;
    }
    ...
    printf("Using AirPrint-style IPP implementation...\n");
    ...
    // AirPrint uses /ipp/print path, not /ipp
    char uri[128];
    snprintf(uri, sizeof(uri), "ipp://%s/ipp/print", ip);
    int uri_len = strlen(uri);
    ...
    printf("JPEG file size: %ld bytes\n", file_size);

    // Build IPP payload exactly like AirPrint
    ipp_payload[offset++] = 0x02; ipp_payload[offset++] = 0x00; // IPP version 2.0 (AirPrint uses 2.0)
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x02; // Print-Job operation
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x00; 
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x01; // Request ID
    ...
    // requesting-user-name (AirPrint always includes this)
    const char *user_name = "iPhone";
    int user_name_len = strlen(user_name);
    ipp_payload[offset++] = 0x42; ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x14;
    memcpy(&ipp_payload[offset], "requesting-user-name", 20); offset += 20;
    ipp_payload[offset++] = (user_name_len >> 8) & 0xFF;
    ipp_payload[offset++] = user_name_len & 0xFF;
    memcpy(&ipp_payload[offset], user_name, user_name_len); offset += user_name_len;

    // job-name (AirPrint format)
    const char *job_name = "Photo";
    int job_name_len = strlen(job_name);
    ipp_payload[offset++] = 0x42; ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x08;
    memcpy(&ipp_payload[offset], "job-name", 8); offset += 8;
    ipp_payload[offset++] = (job_name_len >> 8) & 0xFF;
    ...
    const char *doc_format = "image/jpeg";
    int doc_format_len = strlen(doc_format);
    ipp_payload[offset++] = 0x49; ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x0f;
    memcpy(&ipp_payload[offset], "document-format", 15); offset += 15;
    ipp_payload[offset++] = (doc_format_len >> 8) & 0xFF;
    ipp_payload[offset++] = doc_format_len & 0xFF;
    memcpy(&ipp_payload[offset], doc_format, doc_format_len); offset += doc_format_len;
    ...
    // copies (AirPrint always sends this)
    ipp_payload[offset++] = 0x21; // integer
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x06;
    memcpy(&ipp_payload[offset], "copies", 6); offset += 6;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x04;
    ...
    // print-quality (AirPrint usually sends this)
    ipp_payload[offset++] = 0x21; // enum
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x0d;
    memcpy(&ipp_payload[offset], "print-quality", 13); offset += 13;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x04;
    ...
    // orientation-requested (AirPrint includes this for photos)
    ipp_payload[offset++] = 0x21; // enum
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x13;
    memcpy(&ipp_payload[offset], "orientation-requested", 21); offset += 21;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x04;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x00;
    ipp_payload[offset++] = 0x00; ipp_payload[offset++] = 0x03; // portrait

    ipp_payload[offset++] = 0x03; // End of attributes

    // Use AirPrint-style HTTP header with /ipp/print path
    static char http_header[256];
    snprintf(http_header, sizeof(http_header),
        "POST /ipp/print HTTP/1.1\r\n"
        "Host: %s\r\n"
        "User-Agent: Amiga-IPP/1.0\r\n"
        "Content-Type: application/ipp\r\n"
        "Content-Length: %d\r\n"
        "Connection: close\r\n\r\n",
        ip, offset + file_size);

    printf("Connecting to printer (AirPrint style)...\n");
    sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (sockfd < 0) {
        printf("Socket creation failed.\n");
        free(file_data);
    ...
    struct timeval timeout = {5, 0}; // Shorter 5-second timeout to see what happens
    setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout, sizeof(timeout));
    ...
    printf("Sending AirPrint-style request...\n");
    printf("HTTP header: %d bytes\n", strlen(http_header));
    printf("IPP payload: %d bytes\n", offset);
    printf("JPEG data: %ld bytes\n", file_size);
    ...
    // Debug: Save AirPrint-style payload
    FILE *dump = fopen("UHD:airprint-payload.bin", "wb");
    if (dump) {
        fwrite(ipp_payload, 1, offset, dump);
        fclose(dump);
        printf("Saved AirPrint payload to UHD:airprint-payload.bin\n");
    }
    ...
    ssize_t received = recv(sockfd, response_buffer, sizeof(response_buffer) - 1, 0);
    if (received <= 0) {
        printf("No response - but job may have been accepted\n");
        CloseSocket(sockfd);
        operation_in_progress = FALSE;
        return 0;
    }
    ...
    // Save response
    FILE *resp_dump = fopen("UHD:airprint-response.txt", "wb");
    if (resp_dump) {
        fwrite(response_buffer, 1, received, resp_dump);
        fclose(resp_dump);
        printf("Saved response to UHD:airprint-response.txt\n");
    }
    ...
    if (ipp_start) {
        ipp_start += 4;
        printf("IPP Status: 0x%02x%02x\n", 
               (unsigned char)ipp_start[4], (unsigned char)ipp_start[5]);
               
        unsigned short status = ((unsigned char)ipp_start[4] << 8) | (unsigned char)ipp_start[5];
        if (status == 0x0000) {
            printf("SUCCESS! AirPrint-style request worked!\n");
        } else {
    ...
    CloseSocket(sockfd);
    operation_in_progress = FALSE;
    printf("AirPrint-style print job completed\n");
    return 0;
}
                ...
                case IDCMP_GADGETUP:
                    switch (gad->GadgetID) {
                        ...
                        case GAD_PRINT_BUTTON:
                        {
                            ...
                            send_print_job_airprint_style(print_ip, print_port, file_buffer, media_str, selected_print_mode);
                            }

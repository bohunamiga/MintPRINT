...
// Stub: load ILBM and convert to RGB24 (packed)
// Replace with real IFF parser if needed
int load_ilbm_to_rgb(const char *filename, unsigned char **rgb_out, int *width_out, int *height_out) {
    ...
    if (!dto) {
        printf("Failed to open ILBM file via datatypes: %s\n", filename);
        return -1;
    }

    DoMethod(dto, DTM_PROCLAYOUT, NULL, TRUE);
    GetDTAttrs(dto,
        PDTA_BitMapHeader, (Tag)&bmhd,
        PDTA_BitMap, (Tag)&bitmap,
        TAG_DONE);

    if (!bmhd || !bitmap) {
        printf("Failed to extract bitmap from datatypes object\n");
        DisposeDTObject(dto);
        return -1;
    }
    ...
    *rgb_out = AllocVec(size, MEMF_ANY);
    if (!*rgb_out) {
        DisposeDTObject(dto);
        return -1;
    }
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ULONG pixel = ReadPixel(&rp, x, y);
            UBYTE r = (pixel >> 16) & 0xFF;
            UBYTE g = (pixel >> 8) & 0xFF;
            UBYTE b = pixel & 0xFF;
            (*rgb_out)[(y * width + x) * 3 + 0] = r;
            (*rgb_out)[(y * width + x) * 3 + 1] = g;
            (*rgb_out)[(y * width + x) * 3 + 2] = b;
        }
    }
